...
async def pipeline_detector(state: EnumerationState) -> EnumerationState:
    await emit_agent_step(
        state,
        phase="pipeline",
        label="Updating pipeline map",
        status="start",
        detail="Synthesizing observed paths into a compact topology.",
    )
    ...
    baseline = _materialize_pipeline(state.get("pipeline_json", {}), state)
    ...
    if runtime_cfg.prefers_tight_loops:
        await emit_agent_step(
            ...
            status="done",
            detail=(
                f"Local fast path: {baseline.get('topology_type', 'unknown')} / "
                f"{len(baseline.get('nodes', []))} nodes"
            ),
        )
        return {
            **state,
            "pipeline_json": baseline,
    ...
    session_brief = compiled_session_brief(
        state,
        max_chars=min(runtime_cfg.compiled_brief_char_budget(), 2600),
    )
    prompt = PIPELINE_DETECTOR_USER.format(
        session_brief=session_brief,
        previous_topology=pipeline_snapshot_digest(state),
    )
    ...
    try:
        raw = await client.chat(
            ...
            max_tokens=runtime_cfg.pipeline_token_budget(settings.max_analysis_tokens),
        )
        ...
        await emit_agent_step(
            ...
            status="done",
            detail=shorten(
                client.last_reasoning
                or f"{data.get('topology_type', 'unknown')} / {len(data.get('nodes', []))} nodes",
                width=140,
                placeholder="...",
            ),
        )
        ...
        if data.get("nodes"):
            ...
            await emit_agent_step(
                ...
                status="done",
                detail=(
                    f"Fallback topology: {data.get('topology_type', 'unknown')} / "
